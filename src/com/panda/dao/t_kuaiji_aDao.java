 ...
 * 15:59
 */
public class t_kuaiji_aDao extends ConnectionDao {

	private static Logger logger = Logger.getLogger(t_kuaiji_aDao.class.toString());
...
//						logger.debug("executeBatch E");
	...
	public LinkedHashMap<String, t_kuaiji_aBean> selectAll_where_yyyymmdd_count_by_yyyy(String yyyymmdd_count, String yyyy) {

		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;
		LinkedHashMap<String, t_kuaiji_aBean> LinkedHashMap_t_kuaiji_aBean = new LinkedHashMap<String, t_kuaiji_aBean>();
		try {
			/*
...
//	  `tax_amount` decimal(10,2) NOT NULL COMMENT '消费税金额',
			...
			int count = 0;
			while (resultSet.next()) {
				t_kuaiji_aBean t_kuaiji_aBean = new t_kuaiji_aBean();
				t_kuaiji_aBean.setUPDATE_DATE(resultSet.getString("UPDATE_DATE").split("\\.")[0]);
				t_kuaiji_aBean.setYyyymmdd_count(resultSet.getString("yyyymmdd_count"));
				t_kuaiji_aBean.setYyyy(resultSet.getString("yyyy"));

				t_kuaiji_aBean.setTransaction_date(String.valueOf(resultSet.getString("transaction_date")));
				t_kuaiji_aBean.setCustomer(String.valueOf(resultSet.getString("customer")));
				t_kuaiji_aBean.setProduct(String.valueOf(resultSet.getString("product")));
				t_kuaiji_aBean.setTotal_amount_with_tax(String.valueOf(resultSet.getDouble("total_amount_with_tax")));

				LinkedHashMap_t_kuaiji_aBean.put("" + ++count, t_kuaiji_aBean);
			}
		...
		return LinkedHashMap_t_kuaiji_aBean;
	}
	...
	private String generateInsertSQL(String yyyy, String[] header) {
		...
		return "INSERT INTO t_kuaiji_a (" + columns + ") VALUES (now(3), " + placeholders + ")";
	}
