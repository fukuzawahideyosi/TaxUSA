...
@WebServlet("/SetUserInfoLogic")
@MultipartConfig
public class SetUserInfoLogic extends HttpServlet {

	private static Logger logger = Logger.getLogger(SetUserInfoLogic.class.toString());
...
//					yyyymmdd_count = m_sequenceDao.selectMax_yyyymmdd_count();
					...
					try {
						...
						//登录信息发邮件给客户
						FuncUtils.sendMail_denglu(t_etax_account_infoBean, path, xiaoshouerYYYY_2_title, xiaoshouerYYYY_1_half_title, tokutei_kikann_siharai_kyuuyo_title, xiaoshouerYYYY_1_YYYYMMDD_title, shuoming);
			...
			req.getRequestDispatcher("/setUserInfo.jsp").forward(req, resp);
		} catch (Exception e) {
			e.printStackTrace();
		}
