...
@WebServlet("/SearchEtaxSendJieguoLogic")
@MultipartConfig
public class SearchEtaxSendJieguoLogic extends HttpServlet {

	private static Logger logger = Logger.getLogger(SearchEtaxSendJieguoLogic.class.toString());
...
//		PrintWriter out = resp.getWriter();
		...
		try {
			...
			LinkedHashMap<String, t_etax_jieguoExExBean> LinkedHashMap_t_etax_jieguoExExBean = t_etax_jieguoDao.selectAll(User_infoBean, maxNo);
...
//				session.setAttribute("LinkedHashMapTongji", LinkedHashMapTongji);
			...
			req.getRequestDispatcher("/SearchEtaxSendJieguo.jsp").forward(req, resp);
		} catch (Exception e) {
			e.printStackTrace();
		}
